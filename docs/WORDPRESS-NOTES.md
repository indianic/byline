...
| `GET /wp/v2/tags?search=ProbeAI` returned **both** `ProbeAI` and `ProbeAI Ethics` — a **substring** match. | Taking the first search result would silently tag a post with the wrong term. `resolveTermIds('tags', ...)` (formerly `resolveTagIds`, kept as a thin wrapper) requires an exact, case-insensitive match before trusting a search hit; only when none exists is a new tag created. |
| `GET /wp/v2/categories?search=` — **UNVERIFIED**: never probed live. `resolveTermIds('categories', ...)` assumes the identical substring-match behaviour measured for `tags` above, because both taxonomies are built on WordPress core's same `WP_REST_Terms_Controller`, but that assumption has not been checked against a real install. Do not promote to measured without a probe. | `PostInput.categories` resolves through the same exact-match-then-create logic as tags, sharing the code path (`resolveTermIds`) rather than duplicating it — so if the substring-match assumption turns out wrong for categories, fixing it in one place fixes both call sites, but the behaviour itself is still unverified for categories specifically. |
