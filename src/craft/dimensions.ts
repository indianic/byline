...
export const HUMAN_TEXTURES = [
  'TEXTURE — Asymmetry. Break the rhythm on purpose. Never let three consecutive paragraphs have the same shape or length. Do the same inside lists: bullets should not all be the same length or all start with the same part of speech. At least one paragraph must be a single sentence standing completely alone — one that carries a new claim, never a restatement of the paragraph before it — and at least one must run five or six lines.',
